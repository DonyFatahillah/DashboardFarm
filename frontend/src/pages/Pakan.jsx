...
import { Plus, Loader2, Calendar, Wheat, Warehouse, FileSpreadsheet, FileText } from 'lucide-react';
...
export default function Pakan() {
  ...
  const handleExportExcel = () => {
    const exportData = data.map(item => ({
      ...
      'Jenis Pakan': item.jenis_pakan,
      'Jumlah (KG)': item.jumlah_kg
    }));
    exportToExcel(exportData, `Penggunaan-Pakan-${new Date().getTime()}`);
  };

  const handleExportPDF = () => {
    const headers = [['Tanggal', 'Kandang', 'Jenis Pakan', 'Jumlah (KG)']];
    ...
    exportToPDF(headers, body, `Penggunaan-Pakan-${new Date().getTime()}`, 'Laporan Penggunaan Pakan');
  };
